...
//  QRScannerDemo
//
//  Created by Liu Jiang on 2017/5/24.
...
- (void)initDataWithView:(UIView *)parentView{
    ...
    //增加条形码扫描
    NSArray *availableTypes = [_output availableMetadataObjectTypes];
    NSMutableArray *setTypes = [NSMutableArray arrayWithCapacity:0];
    if ([availableTypes containsObject:AVMetadataObjectTypeEAN13Code]) {
           [setTypes addObject:AVMetadataObjectTypeEAN13Code];
       }
    if ([availableTypes containsObject:AVMetadataObjectTypeEAN8Code]) {
           [setTypes addObject:AVMetadataObjectTypeEAN8Code];
       }
    if ([availableTypes containsObject:AVMetadataObjectTypeCode128Code]) {
           [setTypes addObject:AVMetadataObjectTypeCode128Code];
       }
    if ([availableTypes containsObject:AVMetadataObjectTypeQRCode]) {
           [setTypes addObject:AVMetadataObjectTypeQRCode];
       }
       _output.metadataObjectTypes = [setTypes copy];
...
+ (UIImage *)scQRCodeForData:(NSData *)data size:(CGSize)size {
    ...
    [outputImage drawInRect:CGRectMake(0,0 , size.width, size.width)];
    //水印图
